...
from model.global_attention import MultiGlobalGraph
from model.vanilla_transformer_encoder import Transformer
...
class GraphAttentionBlock(nn.Module):
    def __init__(self, adj, input_dim, output_dim, p_dropout):
        ...
        self.global_graph_layer = MultiGlobalGraph(adj2, input_dim, input_dim//4, dropout=p_dropout)
...
class SpatioTemporalModel(SpatioTemporalModelBase):
    ...
    def __init__(self, adj, num_joints_in, in_features, num_joints_out,
        ...
        self.expand_conv2 = nn.Conv2d(in_features, channels, (1, 1), bias=False)
        nn.init.kaiming_normal_(self.expand_conv.weight)
        nn.init.kaiming_normal_(self.expand_conv2.weight)
        ...
        self.Transformer = Transformer(1, channels, 2*channels, length=27)
        self.encoder = nn.Sequential(
            nn.Conv1d(34, channels, kernel_size=1),
            nn.BatchNorm1d(channels, momentum=0.1),
            nn.ReLU(inplace=True),
            nn.Dropout(0.25)
        )
        self.encoder2 = nn.Sequential(
            nn.Conv1d(channels, 34, kernel_size=1),
            nn.BatchNorm1d(34, momentum=0.1),
            nn.ReLU(inplace=True),
            nn.Dropout(0.25)
        )
...
class SpatioTemporalModelOptimized1f(SpatioTemporalModelBase):
    ...
    def __init__(self, adj, num_joints_in, in_features, num_joints_out,
        ...
        self.expand_conv = nn.Conv2d(in_features, channels, (filter_widths[0], 1), stride=(filter_widths[0], 1), bias=False)#应该是膨胀卷积那边的
        self.expand_conv2 = nn.Conv2d(in_features, channels, (1, 1), stride=(1, 1), bias=False)
    ...
    def _forward_blocks(self, x):
        ...
        #128, 2, 27, 17
        '''
        x = torch.cat([x[:,:, 3:, :], x[:,:, :3, :]], 2)
        print(x.shape)
        x = self.relu(self.expand_bn(self.expand_conv2(x)))
        print(x.shape)
        x = torch.cat([x[:,:, -3:, :], x[:,:, :-3, :]], 2)
        print(x.shape)
        '''
